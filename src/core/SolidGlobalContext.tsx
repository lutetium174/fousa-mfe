...
const SolidGlobalContext = createContext<GlobalContextValue>();

export const useGlobalContext = () => {
  const context = useContext(SolidGlobalContext);
  if (!context) {
...
export const GlobalProvider: ParentComponent = (props) => {
  ...
  const value: GlobalContextValue = {
    state,
    setState: (fn) => {
      globalContext.update(fn);
    },
  };

  return (
    <SolidGlobalContext.Provider value={value}>
      {props.children}
